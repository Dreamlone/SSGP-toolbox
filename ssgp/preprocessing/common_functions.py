...
def convert_path_into_string(path: Union[str, Path]):
...
def _step(matrix, biome_matrix, gap):
    """
    A function that defines all the transformations that must occur with the
    matrix
    """
    # Assign a gap value to the matrix with biomes in those places where there are clouds at the momen
...
def cellular_expand(matrix, biome_matrix, gap = -100.0, iter = 10):
    ...
    for iteration in range(0, iter):
        matrix = _step(matrix, biome_matrix, gap)
    return matrix
